...
const ScanItems = ({ itemCode, setItemCode, warehouseCode, cardCode, setWarningMsg }) => {
  ...
  const { customer, isOneTimeCustomer, isCODCustomer, setWarehouseCode,
    salesQuotationItems, setSalesQuotationItem, salesHeader, setSalesQuotationHeader,
  ...
  const handleScan = async (scannedItem) => {
    ...
    await handleItemScan (scannedItem, salesQuotationItems, setSalesQuotationItem, updateSalesQuotationItem, taxProp,
