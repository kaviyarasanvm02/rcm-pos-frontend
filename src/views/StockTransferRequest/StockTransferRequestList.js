...
import { draftStatus as draftStatusList, userRoles, isModuleApprovalsEnabled } from "../../config/config";
...
class StockTransferRequestList extends React.Component {
  ...
  getTransferRequestRecords = async () => {
    console.log("Index - getTransferRequestRecords() this.state.draftStatus: "+ this.state.draftStatus);
    ...
    try {
      ...
      if (Array.isArray(response.data) && response.data.length) {
        if(draftStatus === "ALL" || !draftStatus)
          filteredStockTransferRequestList = response.data;
        else {
  ...
  handleStatusChange = (event) => {
    ...
    let filteredStockTransferRequestList = [];
    if(draftStatus === "ALL")
      filteredStockTransferRequestList = allStockTransferRequestList;
    else {
  ...
  async componentDidMount() {
    ...
    console.log("StockTransferRequestList - componentDidMount draftStatus- "+this.props.draftStatus);
    /** 
              ...
              <Card className="bg-white shadow">
                  ...
                  <Row className="text-left mb-3">
                    ...
                    <Col md="2" className="mt-4 pt-1 text-right">
                      <Button size="sm" color="primary" type="button"
                        onClick={() => this.props.setGRPODraftDetails("Create", null, 3, this.props.userRole)}
                      >
                        Create New Request
                      </Button>
                    </Col>
                ...
                */}
                ...
                <Table size="sm" className="align-items-center table-flush" responsive>
                  ...
                  <tbody>
                    {(Array.isArray(this.state.filteredStockTransferRequestList) && this.state.filteredStockTransferRequestList.length) ? (
                          ...
                          <tr key={request.DocEntry}>
                            <th className="mb-0 text-sm" scope="row">
                              <a style={{cursor: "pointer", textDecoration: "underline"}} 
                                onClick={ request.U_DraftStatus === draftStatusList.PENDING
                                  ? () => this.props.setGRPODraftDetails("Edit", request, 3, this.props.userRole)
                                  : () => this.props.setGRPODraftDetails("View", request, 3, this.props.userRole)
                                }
                              >
                                {request.DocEntry}
                              </a>
                            ...
                            <>
                              ...
                              <>
                                ...
                                <td>{request.U_NoOfApprovals ? request.U_NoOfApprovals : " NA" }</td>
                              </>}
                            </>}
                          </tr>
                        )
